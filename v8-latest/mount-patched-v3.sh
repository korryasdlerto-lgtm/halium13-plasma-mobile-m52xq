...
# (default)".
chroot "${R}" sh -c 'for f in libandroidicu.so libicu.so libicui18n.so libicuuc.so; do [ -f "/system/lib64/$f" ] || cp "/apex/com.android.i18n/lib64/$f" "/system/lib64/$f" 2>/dev/null; done' 2>&1
