...
# FOUND 2026-07-29: relying on usb-moded.service's own
# Environment=LD_LIBRARY_PATH=... drop-in (zz-hybris-bridge-libs.conf) and
# on a persistent /etc/ld.so.cache (rebuilt via ldconfig at install time)
# both turned out unreliable in practice -- getprop (called a few lines
# below via ubports-usb-moded-configurator) still failed live with
# "cannot open shared object file: libhybris-common.so.1" even with both
# of those in place, almost certainly because the /etc copy-to-tmpfs
# trick further down in this same script (needed for the read-only-root
# workaround) doesn't reliably carry the ld.so.cache state through intact
# on every crash-loop restart. Export explicitly, right here, so the
# exec'd configurator (and everything IT execs, including getprop)
# inherits it directly from THIS process -- no dependency on systemd
# unit environment, ld.so.cache, or the /etc tmpfs copy timing.
export LD_LIBRARY_PATH="/opt/halium-lxc-bridge/system-lib64:/opt/halium-lxc-bridge/vendor-lib64${LD_LIBRARY_PATH:+:$LD_LIBRARY_PATH}"
export HYBRIS_LD_LIBRARY_PATH="/opt/halium-lxc-bridge/system-lib64:/opt/halium-lxc-bridge/vendor-lib64${HYBRIS_LD_LIBRARY_PATH:+:$HYBRIS_LD_LIBRARY_PATH}"
