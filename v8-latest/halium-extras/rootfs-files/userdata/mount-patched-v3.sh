...
# НАЙДЕНО 2026-08-18 (в6, фикс WiFi/RIL/netd/медиа-кодека): namespace
# "default" линковщика (используется /vendor/bin/hw/* и другими
# не-/system бинарями) не может разрешить libandroidicu.so через свой
# обычный редирект в namespace com_android_i18n -- НО ТОЛЬКО для non-root
# UID сервисов (wifi, radio, ...). Живьём подтверждено: идентичный
# бинарь/идентичный linkerconfig успешно линкуется от root, падает от
# uid=wifi(1010) -- проверено и через lxc-attach --uid, и через
# strace -u, с исключёнными SELinux/правами/capabilities/resource-limits.
# Точная причина внутри bionic не найдена несмотря на глубокую живую
# диагностику. Практический обходной путь: скопировать недостающие ICU
# библиотеки прямо в /system/lib64 (до-APEX схема размещения), чтобы
# собственный search path namespace "default" находил их напрямую, без
# необходимости в кросс-namespace редиректе, который падает для non-root
# UID. Именно это живьём починило падавшие в цикле vendor.wifi_hal_legacy,
# ril-daemon, vendor.ipacm и vendor-qti-media-c2-hal-1-0 -- у всех была
# идентичная ошибка "library libandroidicu.so not found ... namespace
# (default)".
# ИСПРАВЛЕНО 2026-08-18 (в6, вторая живая проверка): "chroot "${R}" sh -c
# ..." выше падал живьём с "chroot: failed to run command 'sh': No such
# file or directory" -- на этом раннем этапе (staging-точка ДО
# pivot_root) /system/bin/sh ещё не гарантированно резолвится внутри
# chroot. Переписано на прямые host-видимые пути "${R}/..." без chroot,
# как и остальные bind-операции в этом скрипте -- apex com.android.i18n
# уже примонтирован циклом выше, так что источник доступен напрямую.
for f in libandroidicu.so libicu.so libicui18n.so libicuuc.so; do
    [ -f "${R}/system/lib64/$f" ] || cp "${R}/apex/com.android.i18n/lib64/$f" "${R}/system/lib64/$f" 2>/dev/null
done

# НАЙДЕНО 2026-08-18 (в6, фикс WPSS/WLAN-радио) -- ОТКАЧЕНО в тот же
# день: изначально здесь был добавлен фикс, вручную биндивший
# rev3-прошивку (за отсутствием rev7-блока в /vendor/etc/init/
# wifi_firmware.rc для SM-M526BR при ro.boot.rp=7). Но при более
# внимательном чтении скрипта нашёлся УЖЕ СУЩЕСТВУЮЩИЙ, более точный
# фикс ниже по файлу (строка ~598, "wpss rev7 firmware... from
# TheMuppets vendor blob repo") -- реальная прошивка ИМЕННО под rev7,
# из /userdata/wpss-rev7/. Мой rev3-фикс выполнялся здесь РАНЬШЕ по
# скрипту, чем та секция, так что при обоих активных получалось бы
# ровно наоборот задуманного -- поздний (правильный, rev7) bind
# перезаписывался бы этим более ранним (rev3, хуже) при следующем
# проходе. Удалено как избыточное и потенциально вредное дублирование.
# WPSS всё ещё не поднимается (state=OFFLINING, icnss stats на нуле)
# даже с правильной rev7-прошивкой -- открытый вопрос, требует
# дальнейшей живой диагностики отдельно от ICU/линковщика.
